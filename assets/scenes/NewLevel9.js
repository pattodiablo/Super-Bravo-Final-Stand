...
class NewLevel9 extends BaseScene {
	...
	editorCreate() {
		...
		new_level9.addTilesetImage("late_woods_tiles", "late_woods_tiles");
		...
		// player
		const player = new Player(this, 190, 673);
		...
		// supaBomb
		const supaBomb = new SupaBomb(this, 1518, 296);
		this.add.existing(supaBomb);

		// supaBomb_1
		const supaBomb_1 = new SupaBomb(this, 2305, 265);
		this.add.existing(supaBomb_1);

		// sampoShooter
		const sampoShooter = new SampoShooter(this, 872, 500);
		this.add.existing(sampoShooter);

		// supaBomb_2
		const supaBomb_2 = new SupaBomb(this, 1022, 262);
		this.add.existing(supaBomb_2);

		// sampoShooter_1
		const sampoShooter_1 = new SampoShooter(this, 2181, 479);
		this.add.existing(sampoShooter_1);

		// catapulta
		const catapulta = new Catapulta(this, 2717, 750);
		this.add.existing(catapulta);

		// misile
		const misile = new Misile(this, 3277, 407);
		this.add.existing(misile);

		// sampoShooter_2
		const sampoShooter_2 = new SampoShooter(this, 339, 121);
		this.add.existing(sampoShooter_2);

		// angrySpin
		const angrySpin = new AngrySpin(this, 3049, 734);
		this.add.existing(angrySpin);

		// heart
		const heart = new Heart(this, 2964, 153);
		this.add.existing(heart);

		// heart_1
		const heart_1 = new Heart(this, 288, 148);
		this.add.existing(heart_1);

		// angrySpin_1
		const angrySpin_1 = new AngrySpin(this, 1605, 545);
		this.add.existing(angrySpin_1);

		// heart_2
		const heart_2 = new Heart(this, 1517, 182);
		this.add.existing(heart_2);

		// mechaPirana
		const mechaPirana = new MechaPirana(this, 760, 858);
		this.add.existing(mechaPirana);

		// enemyCreator
		const enemyCreator = new EnemyCreator(this, 761, 873);
		this.add.existing(enemyCreator);

		// drone
		const drone = new Drone(this, 1623, 657);
